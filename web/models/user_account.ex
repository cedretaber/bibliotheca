defmodule Bibliotheca.UserAccount do
  ...
  def changeset(param), do:
    ...
    |> validate_relation(param["account_id"] || param[:account_id], param["user_id"] || param[:user_id])

  def own?(user_id, account_id), do:
    !!Repo.get_by(__MODULE__, account_id: account_id, user_id: user_id)

  def create(user_id, account_id), do:
    Repo.insert changeset(%{account_id: account_id, user_id: user_id})
  ...
  defp validate_relation(changeset, account_id, user_id) do
    ...
    with {:ok, _} <- find_account(account_id),
         ...
         nil      <- Repo.get_by(__MODULE__, account_id: account_id, user_id: user_id)
    do
      changeset
    else
