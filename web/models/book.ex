defmodule Bibliotheca.Book do
  ...
  def all, do: Repo.all(present_books_query())

  def create(params), do: Repo.insert(changeset(params))

  def find(id), do: Repo.one(from(b in present_books_query(), where: b.id == ^id))
  ...
  def search(keyword) do
    ...
    q =
      from(
        b in present_books_query(),
        left_join: ba in "book_authors",
        on: b.id == ba.book_id,
        left_join: a in Author,
        on: a.id == ba.author_id,
        where:
          like(a.name, ^k) or like(b.title, ^k) or like(b.description, ^k) or
            like(b.publisher, ^k) or like(b.isbn, ^k),
        distinct: b.id,
        preload: :authors,
        order_by: [asc: b.id]
      )
  ...
  def remove(book_id), do: Repo.insert(BookRemoved.changeset(%{book_id: book_id}))

  defp present_books_query,
    do:
      from(
        b in __MODULE__,
  ...
  defp cast_authors(author_names),
    do:
      author_names
      |> Enum.map(fn name ->
