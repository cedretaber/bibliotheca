defmodule Bibliotheca.Api.BookView do
  ...
  def render("index.json", %{books: books}),
    do: %{books: render_many(books, __MODULE__, "book.json")}

  def render("show.json", %{book: book}), do: %{book: render_one(book, __MODULE__, "book.json")}

  def render("book.json", %{book: book}),
    do: %{
      id: book.id,
      title: book.title,
