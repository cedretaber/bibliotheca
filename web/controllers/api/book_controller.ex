defmodule Bibliotheca.Api.BookController do
  ...
  def index(conn, %{"q" => q}), do: render(conn, :index, books: Book.search(q))
  def index(conn, _param), do: index(conn, %{"q" => ""})

  def lending(conn, %{"id" => id}),
    do: json(conn, %{account_id: (account = BookLent.lending_account(id)) && account.id})
  ...
  def create(conn, %{"book" => book_param}), do: show_book(conn, Book.create(book_param))

  def remove(conn, %{"id" => id}), do: resp_no_contents(conn, Book.remove(id))
