defmodule Bibliotheca.BookControllerTest do
  ...
  describe "index/2" do
    test "without query all books.", %{conn: conn} do
      ...
      assert json_response(conn, 200)["books"] |> Enum.sort_by(& &1["id"]) ==
               (BookView.render("index.json", %{
                  books:
                    for(
                      book <- [@book1, @book2, @book3],
                      do:
                        Repo.one(
                          from(b in Book, where: b.id == ^book.id, preload: [:authors], select: b)
                        )
                    )
                })
                |> jsonise())["books"]
    ...
    test "search one book", %{conn: conn} do
      ...
      assert json_response(conn, 200)["books"] |> Enum.sort_by(& &1["id"]) ==
               (BookView.render("index.json", %{
                  books: [
                    Repo.one(
                      from(b in Book, where: b.id == ^@book2.id, preload: [:authors], select: b)
                    )
                  ]
                })
                |> jsonise())["books"]
  ...
  describe "create/2" do
    test "create a book.", %{conn: conn} do
      ...
      for attr <- ~w(title description publisher image_url isbn page_count)a,
          do: assert({:ok, params[attr]} == Map.fetch(book, attr))
    ...
    test "create a minimal book.", %{conn: conn} do
      ...
      for attr <- ~w(description publisher image_url isbn page_count published_at)a,
          do: assert({:ok, nil} == Map.fetch(book, attr))
    ...
    test "create an invalid book.", %{conn: conn} do
      ...
      assert json_response(conn, 400) ==
               %{
                 errors: [
                   %{title: %{message: "can't be blank", details: [%{validation: :required}]}}
                 ]
               }
               |> jsonise()
    end
    ...
    test "there are no problem with a camelCase param.", %{conn: conn} do
      ...
      for attr <- ~w(title description publisher imageUrl isbn pageCount)a,
          do:
            assert(
              {:ok, params[attr]} ==
                Map.fetch(book, String.to_atom(Macro.underscore(to_string(attr))))
            )
